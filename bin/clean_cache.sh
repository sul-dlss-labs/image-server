...
# Adapted from https://github.com/loris-imageserver/loris/blob/9861c6f3882cc379ab0880a19604cecc933e76f8/bin/loris-cache_clean.sh
#
# Cron script for maintaining the riiif cache size.
#
# CAUTION - This script deletes files. Be careful where you point it!
#

LOG="/opt/app/imageserver/imageserver/current/log/clean_cache.log"

echo -ne "$(date +[%c]) " >> $LOG
echo "starting" >> $LOG

# Check that the image cache directory...
IMG_CACHE_DIR="/opt/app/imageserver/imageserver/current/tmp/cache"
...
current_usage () {
	...
	# quota -Q -u riiif | grep sdb1 | awk '{ print $2 }' # ...much faster!!
...
while [ $usage -gt $REDUCE_TO ] && [ $max_age -ge -1 ]; do
	...
	#tmpfile=/tmp/riiif-cache-clean-$max_age.tmp
